...
class WooCommerce_DMBPay_Card_Gateway extends WC_Payment_Gateway {

	protected $url = 'https://api.dmbpay.com/v1/request-payment';
	protected $testmode;
	protected $application_key;
	protected $secret_key;

	public function __construct() {
		$this->id           = 'woocommerce_dmbpay_card_gateway';
		$this->method_title = __( 'DMBPay Gateway (Card)', 'dmbpay-card-gateway' );
		$this->title        = 'DMBPay (Card)';

		$this->method_description = __( 'Accept card payments using DMBPay\'s Payment Gateway.', 'dmbpay-card-gateway' );
	...
	public function init_form_fields() {
		$this->form_fields = [
			'enabled'              => [
				...
				'label'       => 'Enable DMBPay Card Gateway',
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no',
			],
	...
	public function process_payment( $order_id ) {
		...
		$order->update_status( 'pending-payment', __( 'Awaiting DMBPay Card Payment', 'dmbpay-card-gateway' ) );
	...
	public function callback_handler() {
		...
		if ( 'card_payment_failed' === $data->act || 'payment_failed' === $data->act ) {
			$order->update_status( 'failed', __( 'DMBPay Card Payment Failed', 'dmbpay-card-gateway' ) );
		}
	}
}
